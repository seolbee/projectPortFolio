...
public class Game {
	...
	public Game(Canvas canvas, Canvas nextBlockCanvas, Canvas HoldBlockCanvas, Label scoreLabel, ObservableList<ScoreVO> list ) {
		this.list = list;
		...
		this.holdGc = this.holdBlockCanvas.getGraphicsContext2D();
		this.holdWidth = this.holdBlockCanvas.getWidth();
		this.holdHeight = this.holdBlockCanvas.getHeight();
	...
	public void render() {
		...
		player.hold(holdGc, holdWidth, holdHeight);
	...
	public void reloadTopScore() {
		...
		//tetris 테이블을 score순으로 내림차순(DESC)정렬하여 그 중 앞에 있는 10개를 가져온다.
		try {
			...
			System.out.println("데이터베이스 접속중 오류발생");
			e.printStackTrace();
		} finally {
			JDBCUtil.close(rs);
