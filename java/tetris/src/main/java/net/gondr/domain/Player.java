...
public class Player {
	...
	private Integer holdBlock;
	private Integer holdColor;
	private boolean holdCheck;
	
	private boolean checkHolding;
	...
	public void hold(GraphicsContext gc, double width, double height) {
		if(!holdCheck) return;
		Color color = colorSet[holdColor];
		Point2D[] block = shape[holdBlock][0]; //다음 블럭 모양
		
		gc.clearRect(0, 0, width, height);
		double x = width / 2;
		double y = height / 2;
		double size = width / 4 - 8;
		if(holdBlock == 0) {
			y -= size;
		}
